...
class TrigramGenerator
  ...
  def generate_text(patterns, number_of_paragraphs = 1, number_of_sentences = 1)
    number_of_paragraphs.times.map do
      generate_paragraph(patterns, number_of_sentences)
    end.join("\n\n")
  end

  def generate_paragraph(patterns, number_of_sentences = 1)
    number_of_sentences.times.map do
      generate_sentence(patterns)
    end.join(' ')
  end

  def generate_sentence(patterns)
